...
public class SummaryPage {
	...
	@FindBy(xpath = "//div[@class='panel panel-saved-card']//descendant::input[@placeholder='Security Code']")
	...
	public void enterDetailsAndPay(String cardNumber, String expiry, String ccv, String cardHolder) {
		...
		// wait.until(ExpectedConditions.textToBePresentInElement(payButton, "PAY
		// NOW"));
		...
		// wait.until(ExpectedConditions.textToBePresentInElement(payButton, "PROCEED TO
		// VERIFICATION"));
...
//			actions6.build().perform();
	...
	public void payWithSavedCard(String ccv) {
		wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@id='targetIframe']")));
		wait.until(ExpectedConditions.frameToBeAvailableAndSwitchToIt("targetIframe"));
		
		Actions actions = new Actions(driver);
		wait.until(ExpectedConditions.visibilityOf(buttonSavedCard));
		actions.moveToElement(buttonSavedCard);
		actions.click();
		actions.build().perform();
		wait.until(ExpectedConditions.visibilityOf(inputSavedCardCcv));
		actions.moveToElement(inputSavedCardCcv);
		actions.click();
		actions.sendKeys(ccv);
		actions.build().perform();
